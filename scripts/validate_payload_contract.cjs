...
function main() {
  ...
  if (!fs.existsSync(demoDir)) {
    console.log('  INFO: no demo/ directory — skipping demo fixture validation');
    console.log('\nPayload contract validation passed.');
    process.exit(0);
  }
  const demoFiles = fs
    .readdirSync(demoDir)
  ...
  if (!demoFiles.length) {
    console.error('INFO: no demo payload fixtures found (expected demo/payload_example*.json)');
    process.exit(1);
  }
