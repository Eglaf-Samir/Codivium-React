/* syntax-theme-vars.js
 * ─────────────────────────────────────────────────────────────
 * Reads cv_syntax_theme from localStorage and applies the
 * codeBg / codeFg colours as CSS custom properties on <html>.
 * Must load before mcq-quiz.js so code blocks are styled on
 * first render. Also re-applies on storage events (settings page
 * changes theme in another tab).
 * ─────────────────────────────────────────────────────────────*/
(function () {
  'use strict';
  ...
  function _applyCodeTheme(key) {
    var theme = null;
    for (var i = 0; i < _THEMES.length; i++) {
      if (_THEMES[i].key === key) { theme = _THEMES[i]; break; }
    }
    if (!theme) theme = _THEMES[0]; // fallback: obsidian-code
    var c = theme.colors;
    var root = document.documentElement;
    root.style.setProperty('--cv-code-bg',      c.codeBg);
    root.style.setProperty('--cv-code-fg',      c.codeFg);
    root.style.setProperty('--cv-code-keyword',  c.keyword  || c.codeFg);
    root.style.setProperty('--cv-code-string',   c.string   || c.codeFg);
    root.style.setProperty('--cv-code-comment',  c.comment  || 'rgba(150,160,180,0.70)');
    root.style.setProperty('--cv-code-number',   c.number   || c.codeFg);
    root.style.setProperty('--cv-code-function', c['function'] || c.codeFg);
    root.style.setProperty('--cv-code-operator', c.operator || c.codeFg);
    root.style.setProperty('--cv-code-variable', c.variable || c.codeFg);
    root.style.setProperty('--cv-code-type',     c.type     || c.codeFg);
  }

  // Apply on load
  try {
    var saved = localStorage.getItem('cv_syntax_theme') || 'obsidian-code';
    _applyCodeTheme(saved);
  } catch (_) {
    _applyCodeTheme('obsidian-code');
  }

  // Apply when settings page changes the theme in another tab
  window.addEventListener('storage', function (e) {
    if (e.key === 'cv_syntax_theme' && e.newValue) {
      _applyCodeTheme(e.newValue);
    }
  });

  // Expose for same-page live preview if needed
  window.CVCodeTheme = { apply: _applyCodeTheme };
}());
