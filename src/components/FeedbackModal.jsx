        ...
        <footer className="fb-foot">
          ...
          <div className="fb-foot-actions">
            <button className="fb-btn ghost" type="button"
              onClick={() => navTo('codivium_insights_embedded.html')}>
              Dashboard
            </button>
            <button className="fb-btn ghost" type="button"
              onClick={() => navTo(result.returnMenuUrl || 'menu-page.html')}>
              Return to Menu
            </button>
            <button className="fb-btn gold" type="button" onClick={() => {
              const targetId  = result.nextExerciseId || result.exerciseId || '';
              const returnUrl = result.returnMenuUrl
                ? encodeURIComponent(result.returnMenuUrl) : '';
              navTo(`editor.html?id=${encodeURIComponent(targetId)}${returnUrl ? `&from=${returnUrl}` : ''}`);
            }}>
