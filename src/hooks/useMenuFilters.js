...
export function toggleFilter(current, value, allOptions) {
  ...
  if (allOptions && allOptions.length > 0 && next.length >= allOptions.length)
    return ["all"];
  return next;
}
...
export function useMenuFilters(exercises) {
  ...
  const [selectedCategories, setSelectedCategories] = useState(
    () => saved.categories || ["all"],
  );
  const [selectedLevels, setSelectedLevels] = useState(
    () => saved.levels || ["all"],
  );
  const [selectedCompleteness, setSelectedCompleteness] = useState(
    () => saved.completeness || ["all"],
  );
  const [selectedExerciseTypes, setSelectedExerciseTypes] = useState(
    () => saved.exerciseTypes || ["all"],
  );
  const [selectedMentalModels, setSelectedMentalModels] = useState(
    () => saved.mentalModels || ["all"],
  );
    ...
    (value, allOptions) => {
      setSelectedCategories((prev) => {
        ...
        savePrefs({
          ...
          mentalModels: selectedMentalModels,
          sortField,
          sortDir,
        });
        return next;
  ...
  const filteredExercises = useMemo(() => {
    ...
    let result = exercises.filter((ex) => {
      const statusKey = (ex.completionStatus || "not_started").replace(
        /-/g,
        "_",
      );
      ...
      if (
        !(
          selectedCompleteness.includes("all") ||
          selectedCompleteness.length === 0
        )
      ) {
        const normStatus = norm(statusKey.replace(/_/g, " "));
        if (
          !selectedCompleteness.some((s) =>
            normStatus.includes(s.replace(/-/g, " ")),
          )
        )
          return false;
      }

      if (!passesFilter(selectedExerciseTypes, ex.exerciseType || ""))
        return false;
      if (!passesArrayFilter(selectedMentalModels, ex.mentalModels || []))
        return false;

      if (search) {
        const nameNorm = norm(ex.name);
        const descNorm = norm(ex.shortDescription || "");
        if (!nameNorm.includes(search) && !descNorm.includes(search))
          return false;
      }
    ...
    result = [...result].sort((a, b) => {
      ...
      if (sortField === "category") {
        ...
        av =
          SORT_STATUS_ORDER[
            norm((a.completionStatus || "").replace(/_/g, " "))
          ] ?? 999;
        bv =
          SORT_STATUS_ORDER[
            norm((b.completionStatus || "").replace(/_/g, " "))
          ] ?? 999;
      } else {
        av = norm(a.name);
        bv = norm(b.name);
