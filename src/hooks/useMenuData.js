...
export function useMenuData() {
  ...
  const load = useCallback(async () => {
    ...
    // window.location.search works correctly in SPA since React Router
    // keeps the real query string in the browser URL bar.
    ...
    if (!token) {
      // No auth — fall back to demo data so page is still useful
      setData(DEMO_DATA);
      setLoading(false);
      return;
    }
    ...
    try {
      ...
      // Graceful fallback: if demo data is available use it silently
      if (window.__CODIVIUM_MENU_DATA__) {
        setData(window.__CODIVIUM_MENU_DATA__);
      } else {
        setError(
