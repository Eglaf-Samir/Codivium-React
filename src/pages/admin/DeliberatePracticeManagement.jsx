...
import {
  getalldeliberatePracticelistByadmin,
  DeleteDeliberatePracticePrepration,
  CreateDeliberatePracticPrepration,
  UpdateDeliberatePracticPrepration,
  AllDeliberatePracticeUnitTestListbyId,
  CreateDeliberatePracticUnitTestAdd,
  DeleteDeliberatePracticUnitTest,
  CreateDeliberatePracticePreprationUsingFile,
  CreateeliberatePracticeUnitTestUsingFile,
  GetDeliberatePracticePrepration,
...
const MODE = 'DELIBERATION';

export default function DeliberatePracticeManagement() {
  ...
  // buffer's content is what gets saved to instructions / miniTutorial.)
  ...
  async function loadAll() {
    setLoading(true);
    const res = await getalldeliberatePracticelistByadmin();
  ...
  async function openEdit(it) {
    ...
    try {
      const res = await GetDeliberatePracticePrepration(it.id);
  ...
  async function submitPrep(e) {
    ...
    try {
      res = (Number(form.id) > 0)
        ? await UpdateDeliberatePracticPrepration(body)
        : await CreateDeliberatePracticPrepration(body);
    } catch (e) { res = e; }
    setSaving(false);
    if (res?.status === 200 && res?.data) {
      await loadAll();
  ...
  async function removePrep(id) {
    const ok = await Swal.fire({
      title: 'Delete this practice?', icon: 'warning', showCancelButton: true,
      confirmButtonText: 'Yes, delete', cancelButtonText: 'Cancel', reverseButtons: true,
    });
    if (!ok.isConfirmed) return;
    const res = await DeleteDeliberatePracticePrepration(id);
    if (res?.status === 401) { authFail(); return; }
  ...
  async function bulkUpload(e) {
    ...
    fd.append('File', file);
    const res = await CreateDeliberatePracticePreprationUsingFile(fd);
    setUploading(false);
  ...
  async function openUnits(item) {
    const res = await AllDeliberatePracticeUnitTestListbyId(item.id);
    if (res?.status === 401) { authFail(); return; }
    if (res?.status === 200 && res?.data) {
  ...
  async function refreshUnits() {
    ...
    const res = await AllDeliberatePracticeUnitTestListbyId(unitDetails.id);
  ...
  async function submitUnit(e) {
    ...
    const body = {
      id: unitForm.id,
      deliberatePracticeid: unitDetails.deliberatePracticeid,
      practiceId: unitDetails.practiceId,
      title: unitForm.title,
      description: unitForm.description,
      codeTemplate: unitForm.codeTemplate,
    };
    let res;
    try { res = await CreateDeliberatePracticUnitTestAdd(JSON.stringify(body)); }
    catch (e) { res = e; }
    setSavingUnit(false);
    if (res?.status === 200 && res?.data) {
  ...
  async function removeUnit(unitId) {
    ...
    if (!ok.isConfirmed) return;
    const res = await DeleteDeliberatePracticUnitTest(
      unitDetails.deliberatePracticeid,
      unitDetails.practiceId,
      unitId,
    );
    if (res?.status === 401) { authFail(); return; }
    if (res?.data === true || res?.data === 'true') {
  ...
  async function bulkUploadUnits(e) {
    ...
    const res = await CreateeliberatePracticeUnitTestUsingFile(fd);
      ...
      <div className="cv-admin-page">
        <header className="cv-admin-header">
          <div className="cv-admin-header-text">
            ...
            <h1 className="cv-admin-title">Deliberate Practice Management</h1>
            <p className="cv-admin-subtitle">Practice exercises and their unit tests.</p>
          </div>
          <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
            <button
              type="button"
              className="cv-admin-btn"
              onClick={() => downloadFile(baseURL + 'Emoji/SampleFile/SampleDeliberatePracticePrepration.xlsx', 'SampleDeliberatePractice.xlsx')}
            >↓ Sample</button>
            ...
            <button type="button" className="cv-admin-btn is-primary" onClick={openNew}>+ New Practice</button>
          </div>
        </header>
        ...
        <section className="cv-admin-surface">
          ...
          <div className="cv-admin-table-wrap">
            {loading ? (
              ...
              <div className="cv-admin-table-empty">{search ? 'No matches.' : 'No practices yet.'}</div>
            ) : (
      ...
      <AdminModal open={editOpen} onClose={closeEdit}>
        <div className="cv-admin-modal" onClick={e => e.stopPropagation()}>
          <div className="cv-admin-modal-head">
            <h3>{Number(form.id) > 0 ? 'Update Practice' : 'Add Practice'}</h3>
